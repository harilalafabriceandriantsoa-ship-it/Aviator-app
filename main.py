...
def run_prediction(seed, client, power=1.0):
    ...
    target = round(random.uniform(2.10, 5.85) * power, 2)
    return {
        "val": target,
...
with t1: # AVIATOR - Namboarina ny interface mba tsy ho banga
    st.markdown("### ✈️ AVIATOR PREDICTOR")
    h_avi = st.text_input("Server Seed / Hash Aviator:")
    c_avi = st.text_input("Client Seed / Ora:")
        ...
        if h_avi:
            ...
            st.markdown(f"""
                <div class="prediction-card">
                    ...
                    <small>Min: {res['min']}x | Accuracy: {res['conf']}%</small>
                </div>
            """, unsafe_allow_html=True)
            st.session_state.history.insert(0, f"Aviator: {res['val']}x")

with t2: # COSMOS - Namboarina ny Jumps
                ...
                with cols[i]:
                    st.markdown(f"""
                        <div class="prediction-card">
                            ...
                            <h2 style="color:#00ffcc;">{res['val']}x</h2>
                            <small>Min: {res['min']}x</small>
                        </div>
                    """, unsafe_allow_html=True)
            st.session_state.history.insert(0, f"Cosmos {tour_id}: {res['val']}x")

with t3: # MINES - Namboarina mba tsy ho tapaka ny kaody
