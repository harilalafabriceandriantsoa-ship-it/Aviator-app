...
# --- 1. CONFIGURATION & PREMIUM NEON UI ---
...
st.markdown("""
    ...
    .target-val { 
        ...
        text-shadow: 0 0 20px #00ffcc; margin: 10px 0;
    }
    
    .acc-badge {
        background: linear-gradient(90deg, #00ffcc, #0088ff);
        color: #010a12; padding: 5px 18px; border-radius: 30px;
        font-weight: 800; font-size: 14px; box-shadow: 0 0 10px #00ffcc;
    }
    ...
    .stButton>button {
        ...
        padding: 12px 30px !important; transition: 0.4s !important;
...
# --- 3. CORE TITAN ALGORITHM (TSY NIKITIHINA) ---
def get_titan_prediction(server_hex, base_ora, game_type):
    time.sleep(1.5) # ANTI-BOT PROTECTION
    results = []
                    ...
                    for p in preds:
                        ...
                        <div class="card">
                            <div style="display:flex; justify-content: space-between; align-items:center;">
                                <span style="font-size:24px; color:#ffcc00; font-weight:900;">⏰ {p['lera']}</span>
                            ...
                            <div style="display: flex; justify-content: space-around; margin-top:25px; text-align:center;">
                                <div><small style="color:#888;">MIN (SAFE)</small><br><b style="font-size:20px;">{p['min']}x</b></div>
                                <div><small style="color:#00ffcc; letter-spacing:1px;">PREDICTED</small><br><div class="target-val">{p['moyen']}x</div></div>
                                <div><small style="color:#888;">MAX (RISK)</small><br><b style="font-size:20px;">{p['max']}x</b></div>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
    ...
    if st.button("💎 REVEAL SAFE PATH", use_container_width=True):
        if m_hex:
            ...
            for idx in range(25):
                ...
                grid += f'<div style="background:{color}; height:65px; border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:30px; border:1px solid rgba(255,255,255,0.05); box-shadow: 0 4px 15px rgba(0,0,0,0.4);">{char}</div>'
