...
def run_prediction(seed, client, power=1.0):
    ...
    target = round(random.uniform(1.65, 4.85) * power, 2)
    return {
        "val": target,
        "min": round(target * 0.85, 2),
        "max": round(target * 1.12, 2),
        "conf": round(random.uniform(96.8, 99.9), 1)
    }
...
with t1: # Naveriko ny Aviator mba tsy ho banga
    st.markdown("### ✈️ AVIATOR ANALYSIS")
    s_avi = st.text_input("Server Seed / Hash Aviator:", key="avi_s")
    c_avi = st.text_input("Client Seed / Ora (HH:MM):", key="avi_c")
    if st.button("🔥 ANALYZE AVIATOR"):
        if s_avi:
            res = run_prediction(s_avi, c_avi)
            st.markdown(f"""
                ...
                with cols[i]:
                    st.markdown(f"""
                        <div class="prediction-card">
                            ...
                            <h2 style="color:#00ffcc;">{res['val']}x</h2>
                            <small>Min: {res['min']}x | Max: {res['max']}x</small>
                        </div>
                    """, unsafe_allow_html=True)
