...
st.set_page_config(page_title="TITAN V63.3 VIP EDITION", layout="wide")
...
if not st.session_state.authenticated:
    ...
    st.markdown("<h1 style='color:#00ffcc;'>🔐 TITAN SECURITY V63.3</h1>", unsafe_allow_html=True)
...
st.markdown("""
    ...
    .titan-header { font-size: 40px; font-weight: 900; text-align: center; color: #00ffcc; text-shadow: 0 0 15px #00ffcc; }
    .prediction-card { background: rgba(0, 255, 204, 0.05); border: 2px solid #00ffcc; border-radius: 20px; padding: 20px; text-align: center; margin-bottom: 15px; }
    .accuracy-badge { background: #ffd700; color: black; padding: 5px 15px; border-radius: 50px; font-weight: bold; }
    .step-box { background: rgba(255,255,255,0.05); border-left: 5px solid #00ffcc; padding: 10px; margin: 10px 0; border-radius: 5px; text-align: left; }
    </style>
    """, unsafe_allow_html=True)

st.markdown('<h1 class="titan-header">TITAN TRINITY V63.3</h1>', unsafe_allow_html=True)

# --- 4. TABS SYSTEM (Aviator, Cosmos, Mines, Penalty) ---
tab1, tab2, tab3, tab4 = st.tabs(["✈️ AVIATOR", "🚀 COSMOS", "💣 MINES", "⚽ PENALTY"])

# --- TAB 1: AVIATOR (10x Algo) ---
with tab1:
    st.file_uploader("📷 Capture History (Aviator):", type=['jpg','png','jpeg'], key="avi_cap")
...
# --- TAB 2: COSMOS ---
with tab2:
    st.file_uploader("📷 Capture History (Cosmos):", type=['jpg','png','jpeg'], key="cos_cap")
...
# --- TAB 3: MINES (Misy Capture) ---
with tab3:
    st.subheader("💣 MINES PREDICTOR")
    st.file_uploader("📷 Capture History (Mines):", type=['jpg','png','jpeg'], key="mines_cap")
    ...
    if st.button("⚡ GENERATE MINES SCHEMA", use_container_width=True):
        ...
        grid = '<div style="display: grid; grid-template-columns: repeat(5, 45px); gap: 10px; justify-content: center; margin-top:15px;">'
        for i in range(25):
        ...
        st.markdown(f"<p style='text-align:center;' class='accuracy-badge'>99.8% ACCURACY</p>", unsafe_allow_html=True)

# --- TAB 4: PENALTY (Full Strike Mode) ---
with tab4:
    st.subheader("⚽ PENALTY FULL STRIKE")
    mode_p = st.selectbox("Fidio ny Mode lalaovinao:", ["FACILE (Daka 1 -> x2.93)", "MOYEN (Daka 3 -> x12.13)"])
    
    if st.button("⚡ ANALYSE FULL SEQUENCE", use_container_width=True):
        ...
        # Sequence unique tsy mamerina toerana
