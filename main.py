...
# --- 5. CORE ALGO IA (TSY NIOVA NY SYNTAX) ---
...
with t2:
    ...
    if st.button("🔥 ANALYZE COSMOS"):
        if h_cos and hex_cos and tour_id:
            # --- ETO ILAY IA MIKAJY NY ELANELANA (DYNAMIC JUMP) ---
            ia_val = int(hashlib.md5(h_cos.encode()).hexdigest()[:2], 16)
            # Ny IA no mifidy ny elanelana miankina amin'ny Hash (tsy fix +1)
            s_1 = (ia_val % 4) + 4   # Ohatra: +4 hatramin'ny +7
            s_2 = (ia_val % 6) + 8   # Ohatra: +8 hatramin'ny +13
            s_3 = (ia_val % 10) + 15 # Ohatra: +15 hatramin'ny +24
