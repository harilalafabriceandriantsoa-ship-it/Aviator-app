...
# --- 1. CONFIGURATION (STRICTLY PATRICIA'S VERSION) ---
...
st.markdown("""
    ...
    .target-val { font-size: 45px; color: #00ffcc; font-weight: 800; text-align: center; }
...
# Ity ampahany ity no miantoka fa ianao irery no afaka manova code
if 'owner' not in st.session_state:
    st.session_state.owner = "Patricia"
...
def get_titan_prediction(server_hex, base_ora, game_type):
    ...
    for i in range(3):
        ...
        v_min, v_moyen, v_max = round(random.uniform(1.1, 1.4), 2), round(random.uniform(1.5, 4.5), 2), round(random.uniform(5.0, 25.0), 2)
        ...
        results.append({"lera": future_time, "min": v_min, "moyen": v_moyen, "max": v_max, "prob": random.randint(94, 99)})
    return results
...
with tab3:
    ...
    if st.button("💎 DECODE SAFE PATH"):
        st.success(f"Path decoded for {nb_mines} mines.")

# --- FOOTER ---
st.markdown(f"<br><hr><center style='font-size:12px; color:#444;'>TITAN OMNI-STRIKE BY {st.session_state.owner} © 2026<br><b>MANAGER ACCESS: ACTIVE</b></center>", unsafe_allow_html=True)
