...
# INITIALISATION DES VARIABLES (SANS TOUCHER AU CODE INITIAL)
...
if not st.session_state.authenticated:
    ...
    code_input = st.text_input("Ampidiro ny kaody:", type="password")
    if st.button("Mivoha"):
...
with st.sidebar:
    ...
    if st.button("🗑️ RESET MY HISTORY"):
        ...
        st.success("Voadio ny tantaranao!")
        time.sleep(1)
        st.rerun()

    st.markdown("---")
    # ADMIN ONLY AREA
    unlock_admin = st.text_input("Admin Key (Secret):", type="password")
    if unlock_admin == st.session_state.access_code:
...
# --- 4. ALGORITHM ENGINE (NO CHANGES MADE) ---
...
# --- MINES TAB (DOUBLE SEED LOGIC) ---
with tab3:
    st.markdown("### 💣 MINES VIP DECODER")
    nb_m = st.select_slider("Isan'ny Mines:", options=[1,2,3,4,5], value=3)
    ...
    if st.button("💎 DECODE SAFE PATH"):
        if s_serv and s_cli:
            ...
            for i in range(25):
                if i in diamond_pos: grid_html += '<div class="mine-cell diamond-cell">💎</div>'
                else: grid_html += '<div class="mine-cell">⬛</div>'
