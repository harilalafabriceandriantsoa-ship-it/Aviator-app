...
with tab1:
    ...
    if st.button("🚀 START AVIATOR ANALYSIS"):
        if hex_avi:
            ...
            st.markdown(f"<div class='prediction-card'><div class='range-text'>MIN: {v_min}x | MAX: {v_max}x</div><h1>{v_moy}x</h1><p>SIGNAL DETECTED</p><p class='accuracy-text'>{acc} Accuracy</p></div>", unsafe_allow_html=True)
...
with tab2:
    ...
    if st.button("🚀 START COSMOS ANALYSIS"):
        if hex_cos:
            ...
            st.markdown(f"<div class='prediction-card'><div class='range-text'>MIN: {v_min}x | MAX: {v_max}x</div><h1 style='color:#ffd700;'>{v_moy}x</h1><p>SIGNAL DETECTED</p><p class='accuracy-text'>{acc} Accuracy</p></div>", unsafe_allow_html=True)
