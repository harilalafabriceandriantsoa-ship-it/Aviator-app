...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="TITAN V64.6 ELITE", layout="wide")
...
if not st.session_state.authenticated:
    ...
    st.markdown("<h1 style='color:#00ffcc;'>🛸 TITAN ELITE V64.6</h1>", unsafe_allow_html=True)
...
st.markdown('<h1 class="titan-header">TITAN OMNI-STRIKE V64.6</h1>', unsafe_allow_html=True)
...
with tab3:
    ...
    if st.button("⚡ SCAN GRID (DYNAMIC)", use_container_width=True):
        ...
        # Grid HTML fixed
        grid_html = '<div style="display: grid; grid-template-columns: repeat(5, 50px); gap: 10px; justify-content: center;">'
        for i in range(25):
